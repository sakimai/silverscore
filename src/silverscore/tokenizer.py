...
def default_bpe():
    ...
    if not os.path.exists(local_path):
        cache_dir = os.path.expanduser("~/.cache/silverscore")
        os.makedirs(cache_dir, exist_ok=True)
        cached = os.path.join(cache_dir, "bpe_simple_vocab_16e6.txt.gz")
        if not os.path.exists(cached):
            import urllib.request
            urllib.request.urlretrieve(_BPE_URL, cached)
        return cached
    return local_path
